...
Promtact detects agentic threat behavior by correlating
...
`cmd/promtact` starts a local HTTP service and serves both API endpoints and the
...
`cmd/promtactl` provides operational helper commands. The first command,
...
Promtact JSONL. Current sources are Sysmon JSON, auditd text, Zeek conn logs, and
Suricata EVE JSON.
...
versioned migrations recorded in `promtact_schema_migrations`. For local
development, `--data` writes a JSON snapshot and restores state on startup.
`promtactl backup` and `promtactl restore` reuse the same snapshot shape for
...
rest of the response workflow. The `promtactl wedge-demo` command exercises the
...
are persisted to Postgres table `promtact_audit_events` in production mode. Each
...
with a private key (`promtactl license issue`), deployments verify it with the

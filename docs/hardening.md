...
This document records the security hardening implemented in Promtact and the
...
- **Session signing key required:** `PROMTACT_SESSION_SECRET` is mandatory when auth
...
- **Validity is re-derived from the event rows at runtime** in the Postgres path
  ...
  flag.
- **HMAC-anchored** with a server-held key (`PROMTACT_AUDIT_HMAC_SECRET`); when only
  `PROMTACT_SESSION_SECRET` is set, the audit key is **derived from it via a labelled
...
- Agents can be registered in `policy.json` (`agent_identities`: `agent_id` +
  ...
  `promtactl token-hash`). When any identity is configured the gateway stops
...
- **Self-hosted runner runs as a non-root user** and may `sudo` only a single
  fixed, root-owned deploy wrapper (`/usr/local/sbin/promtact-deploy`) — not
...
- [ ] `PROMTACT_SESSION_SECRET` — 32+ random bytes (required with auth/SSO).
- [ ] `PROMTACT_AUDIT_HMAC_SECRET` — dedicated key for the audit anchor (recommended;
...
- [ ] systemd unit installed from `packaging/systemd/` (binds loopback + full
      sandbox); secrets in `/etc/promtact/<instance>.env` (mode 0600); reach the UI
      via reverse proxy or SSH tunnel.
- [ ] `PROMTACT_MANIFEST_HMAC_SECRET` set if you load a custom threat pack (signing
      then fails closed).

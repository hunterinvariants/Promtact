...
- SHA-256 hash chain `H(prev || record)`; chain head shared in Postgres via
  ...
  server-held key (`PROMTACT_AUDIT_HMAC_SECRET`); snapshot reports `unlinked` and
...
- [ ] `PROMTACT_AUDIT_HMAC_SECRET` — dedicated key for the audit anchor (do not reuse
      the session secret).
...
- [ ] systemd unit installed from `packaging/systemd/`; secrets in
      `/etc/promtact/<instance>.env` (mode 0600).

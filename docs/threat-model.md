...
This document is the threat model for Promtact (Promtact). It
...
Promtact is a defensive control plane for agentic threat behavior. Its security-
...
2. **Authenticated client → API:** authenticated principals are bounded by RBAC
   ...
   input).
3. **Promtact → outbound connectors:** destination URLs are operator-configured;
   the client-supplied gateway-proxy upstream is validated and IP-pinned.
4. **Promtact → datastore:** the store is trusted for availability but the audit

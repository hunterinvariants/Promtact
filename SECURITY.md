...
Promtact (Promtact) is defensive security software. Reports
...
- detection gaps: a realistic agent threat pattern the inline gateway fails to
  flag or block. The benign emulation library shipped with `promtactl validate`
...
```bash
promtactl validate --url https://your-promtact.example --token "$PROMTACT_API_TOKEN"
```
...
`PROMTACT_SESSION_SECRET` and a dedicated `PROMTACT_AUDIT_HMAC_SECRET` set, and the
...
base=https://github.com/hunterinvariants/promtact/releases/download/vX.Y.Z
curl -sLO "$base/SHA256SUMS"
curl -sLO "$base/SHA256SUMS.bundle"
curl -sLO "$base/promtact-linux-amd64"   # plus any other artifacts you use
...
sha256sum -c SHA256SUMS 2>/dev/null | grep -E 'promtact|SBOM'

# 4) (optional) Confirm the binary reports the expected version
chmod +x promtact-linux-amd64
timeout 2 ./promtact-linux-amd64 --addr 127.0.0.1:0 --demo 2>&1 | grep -o 'Threat Defense [0-9.]*'
```

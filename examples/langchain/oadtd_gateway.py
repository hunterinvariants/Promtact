"""Minimal Promtact inline-gateway client — gate an agent tool call before it runs.
...
guard on top of this; this module is what actually talks to Promtact and is fully
...
class ToolBlocked(Exception):
    """Raised when Promtact does not allow a tool call."""

    def __init__(self, verdict: str, reason: str) -> None:
        self.verdict = verdict
        self.reason = reason
        super().__init__(f"Promtact blocked tool call ({verdict}): {reason}")
...
class PromtactGateway:
    """Calls the Promtact inline gateway's read-only decide endpoint."""

"""Tests for the Promtact gateway client against a stub server.

Standard library only — no LangChain, no LLM, no Promtact binary required:
...
from promtact_gateway import ALLOW, PromtactGateway, ToolBlocked
...
class GatewayClientTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.thread.start()
        cls.gateway = PromtactGateway(url=f"http://127.0.0.1:{port}")

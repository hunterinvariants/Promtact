"""Runnable example: a LangChain agent whose tools are gated by Promtact.
...
Prereqs:
    ...
    # an Promtact server with an api-token configured and the demo tools approved:
...
from promtact_gateway import PromtactGateway, ToolBlocked
...
def main() -> None:
    gateway = PromtactGateway(
        url=os.environ.get("PROMTACT_URL", "http://127.0.0.1:8080"),
        token=os.environ.get("PROMTACT_TOKEN"),

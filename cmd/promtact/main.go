...
func main() {
	...
	threatPackPath := flag.String("threat-pack", os.Getenv("PROMTACT_THREAT_PACK"), "optional threat pack JSON file")
	deceptionTokensPath := flag.String("deception-tokens", os.Getenv("PROMTACT_DECEPTION_TOKENS"), "optional JSON file of deception/canary tokens")
	tenantPoliciesPath := flag.String("tenant-policies", os.Getenv("PROMTACT_TENANT_POLICIES"), "optional JSON file of org-scoped policy sets")
	licenseFile := flag.String("license-file", os.Getenv("PROMTACT_LICENSE_FILE"), "path to a commercial license token file")
	licensePublicKey := flag.String("license-public-key", os.Getenv("PROMTACT_LICENSE_PUBLIC_KEY"), "base64 ed25519 public key to verify the license")
	...
	jiraBaseURL := flag.String("jira-base-url", os.Getenv("PROMTACT_JIRA_BASE_URL"), "Jira base URL for incident tickets")
	jiraEmail := flag.String("jira-email", os.Getenv("PROMTACT_JIRA_EMAIL"), "Jira account email")
	jiraAPIToken := flag.String("jira-api-token", os.Getenv("PROMTACT_JIRA_API_TOKEN"), "Jira API token")
	jiraProjectKey := flag.String("jira-project-key", os.Getenv("PROMTACT_JIRA_PROJECT_KEY"), "Jira project key for incidents")
	jiraIssueType := flag.String("jira-issue-type", os.Getenv("PROMTACT_JIRA_ISSUE_TYPE"), "Jira issue type (default Task)")
	servicenowURL := flag.String("servicenow-url", os.Getenv("PROMTACT_SERVICENOW_URL"), "ServiceNow instance URL for incidents")
	servicenowUser := flag.String("servicenow-user", os.Getenv("PROMTACT_SERVICENOW_USER"), "ServiceNow user")
	servicenowPassword := flag.String("servicenow-password", os.Getenv("PROMTACT_SERVICENOW_PASSWORD"), "ServiceNow password")
	mcpUpstreamURL := flag.String("mcp-upstream-url", os.Getenv("PROMTACT_MCP_UPSTREAM_URL"), "upstream MCP server URL for transparent interception")
	mcpUpstreamToken := flag.String("mcp-upstream-token", os.Getenv("PROMTACT_MCP_UPSTREAM_TOKEN"), "optional bearer token for MCP upstream")
	proxyAllowLocalTargets := flag.Bool("proxy-allow-local-targets", parseBoolEnv(os.Getenv("PROMTACT_PROXY_ALLOW_LOCAL_TARGETS")), "allow the gateway proxy to reach loopback/private/internal upstreams (DANGEROUS; off by default)")
	oidcIssuerURL := flag.String("oidc-issuer-url", os.Getenv("PROMTACT_OIDC_ISSUER_URL"), "OIDC issuer URL for SSO login")
	oidcClientID := flag.String("oidc-client-id", os.Getenv("PROMTACT_OIDC_CLIENT_ID"), "OIDC client ID")
	oidcClientSecret := flag.String("oidc-client-secret", os.Getenv("PROMTACT_OIDC_CLIENT_SECRET"), "OIDC client secret")
	oidcRedirectURL := flag.String("oidc-redirect-url", os.Getenv("PROMTACT_OIDC_REDIRECT_URL"), "OIDC redirect URL")
	oidcScopes := flag.String("oidc-scopes", defaultString(os.Getenv("PROMTACT_OIDC_SCOPES"), "openid,profile,email"), "comma-separated OIDC scopes")
	oidcTenantClaim := flag.String("oidc-tenant-claim", os.Getenv("PROMTACT_OIDC_TENANT_CLAIM"), "OIDC claim name for tenant assignment")
	oidcRoleClaim := flag.String("oidc-role-claim", os.Getenv("PROMTACT_OIDC_ROLE_CLAIM"), "OIDC claim name for roles")
	oidcEmailClaim := flag.String("oidc-email-claim", os.Getenv("PROMTACT_OIDC_EMAIL_CLAIM"), "OIDC claim name for user name/email")
	samlRootURL := flag.String("saml-root-url", os.Getenv("PROMTACT_SAML_ROOT_URL"), "SAML service provider root URL")
	samlIDPMetadataURL := flag.String("saml-idp-metadata-url", os.Getenv("PROMTACT_SAML_IDP_METADATA_URL"), "SAML identity provider metadata URL")
	samlKeyPath := flag.String("saml-key-path", os.Getenv("PROMTACT_SAML_KEY_PATH"), "SAML signing key path")
	samlCertPath := flag.String("saml-cert-path", os.Getenv("PROMTACT_SAML_CERT_PATH"), "SAML signing certificate path")
	samlTenantAttribute := flag.String("saml-tenant-attribute", os.Getenv("PROMTACT_SAML_TENANT_ATTRIBUTE"), "SAML attribute name for tenant assignment")
	samlRoleAttribute := flag.String("saml-role-attribute", os.Getenv("PROMTACT_SAML_ROLE_ATTRIBUTE"), "SAML attribute name for roles")
	samlEmailAttribute := flag.String("saml-email-attribute", os.Getenv("PROMTACT_SAML_EMAIL_ATTRIBUTE"), "SAML attribute name for user name/email")
	publicURL := flag.String("public-url", os.Getenv("PROMTACT_PUBLIC_URL"), "public URL for HA and SSO callbacks")
	instanceName := flag.String("instance-name", os.Getenv("PROMTACT_INSTANCE_NAME"), "instance name for HA deployments")
	tenantIsolationMode := flag.String("tenant-isolation-mode", defaultString(os.Getenv("PROMTACT_TENANT_ISOLATION_MODE"), "logical"), "tenant isolation mode: logical or physical")
	tenantRegistryPath := flag.String("tenant-registry-path", os.Getenv("PROMTACT_TENANT_REGISTRY_PATH"), "path to tenant registry JSON")
	tenantPostgresDSNTemplate := flag.String("tenant-postgres-dsn-template", os.Getenv("PROMTACT_TENANT_POSTGRES_DSN_TEMPLATE"), "Postgres DSN template for physical tenant stores")
	tenantDataPathTemplate := flag.String("tenant-data-path-template", os.Getenv("PROMTACT_TENANT_DATA_PATH_TEMPLATE"), "file path template for physical tenant stores")
	trustedProxies := flag.String("trusted-proxies", os.Getenv("PROMTACT_TRUSTED_PROXIES"), "comma-separated list of trusted proxy CIDRs or IPs")
	retentionWindow := flag.String("retention-window", defaultString(os.Getenv("PROMTACT_RETENTION_WINDOW"), "30d"), "retention window for events, alerts, actions, and audits")
	...
	if authConfigured && strings.TrimSpace(os.Getenv("PROMTACT_SESSION_SECRET")) == "" {
		log.Fatal("PROMTACT_SESSION_SECRET is required when authentication or SSO is configured")
	}
	...
	log.Printf("Promtact %s listening on http://%s", server.Version, actualAddr)
	srv := &http.Server{
		Addr:              actualAddr,
		Handler:           app.Routes(),

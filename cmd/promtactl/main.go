...
func main() {
	...
	case "tenant":
		if err := tenantCommand(os.Args[2:]); err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
	case "audit":
		if err := auditCommand(os.Args[2:]); err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
	case "asset":
	...
	case "credential":
		if err := credentialCommand(os.Args[2:]); err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
	case "preflight":
...
func tokenHash(args []string) error {
	...
	value := *token
	if value == "" {
		value = os.Getenv("PROMTACT_TOKEN")
	}
	if value == "" {
...
func licenseCommand(args []string) error {
	if len(args) < 1 {
		return errors.New("usage: promtactl license <keygen|issue|verify> ...")
	}
	switch args[0] {
	case "keygen":
	...
	case "issue":
		...
		privateKey := fs.String("private-key", os.Getenv("PROMTACT_LICENSE_PRIVATE_KEY"), "base64 ed25519 private key")
		...
		if strings.TrimSpace(*privateKey) == "" {
			return errors.New("license issue requires --private-key or PROMTACT_LICENSE_PRIVATE_KEY")
		}
	...
	case "verify":
		...
		publicKey := fs.String("public-key", os.Getenv("PROMTACT_LICENSE_PUBLIC_KEY"), "base64 ed25519 public key")
...
func backup(args []string) error {
	...
	dsn := fs.String("postgres-dsn", os.Getenv("PROMTACT_POSTGRES_DSN"), "Postgres DSN")
	outputPath := fs.String("output", "-", "backup JSON file, or - for stdout")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if *dsn == "" {
		return errors.New("backup requires --postgres-dsn or PROMTACT_POSTGRES_DSN")
	}
...
func restore(args []string) error {
	...
	if *dsn == "" {
		return errors.New("restore requires --postgres-dsn or PROMTACT_POSTGRES_DSN")
	}
	if *inputPath == "" {
...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "  promtactl credential list | set --tool NAME | remove --id ID")
	...
	fmt.Fprintln(os.Stderr, "  promtactl agent --source windows-eventlog [--log-name Microsoft-Windows-Sysmon/Operational]")
	fmt.Fprintln(os.Stderr, "  promtactl agent --source journald [--journal-unit ssh.service]")
	fmt.Fprintln(os.Stderr, "  promtactl token-hash --token TOKEN")
	fmt.Fprintln(os.Stderr, "  promtactl wedge-demo [--url http://localhost:8080] [--approved-by operator] [--await-approval]")
	fmt.Fprintln(os.Stderr, "  promtactl validate [--url http://localhost:8080] [--token TOKEN] [--json]")
	fmt.Fprintln(os.Stderr, "  promtactl mcp-stub [--addr 127.0.0.1:9100]")

...
// mcpDemoCommand drives a curated sequence of MCP tool calls through the Promtact
...
// client — start `promtactl mcp-stub`, run Promtact with --mcp-upstream-url pointing
...
func mcpDemoCommand(args []string) error {
	...
	if *jsonOut {
		...
		fmt.Println("promtactl mcp-demo — live MCP enforcement through the gateway proxy")
		for _, r := range rows {
			status := "PASS"
			if !r.Pass {
				status = "FAIL"

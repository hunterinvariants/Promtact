...
```powershell
$env:PROMTACT_API_TOKEN="replace-with-a-local-secret"
...
```powershell
go run ./cmd/promtactl replay --file examples\demo-events.jsonl
```
...
--api-token  optional token for POST endpoints, defaults to PROMTACT_API_TOKEN
```

When `--api-token` or `PROMTACT_API_TOKEN` is set, read endpoints remain available
for the dashboard and health checks, while write endpoints require
`Authorization: Bearer <token>` or `X-Promtact-Token: <token>`.

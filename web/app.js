...
async function api(path, options = {}) {
  ...
  const token = useToken ? sessionStorage.getItem("promtact_api_token") : "";
  if (token) {
...
function handleApiFailure(error) {
  if (error && error.status === 401) {
    sessionStorage.removeItem("promtact_api_token");
...
function renderGraph(assets, alerts) {
  ...
  els.graph.innerHTML = `
    ...
    <text x="${width / 2}" y="${height / 2 - 2}" text-anchor="middle" fill="#fff" font-size="12" font-weight="800">Promtact</text>

...
func New(users []UserConfig, legacyToken string) *Authenticator {
	...
	authenticator.sessionKey = deriveSessionKey(authenticator.users, authenticator.legacyHash, os.Getenv("PROMTACT_SESSION_SECRET"))
...
func readToken(r *http.Request) string {
	...
	return strings.TrimSpace(r.Header.Get("X-Promtact-Token"))
}
...
const sessionCookieName = "promtact_session"

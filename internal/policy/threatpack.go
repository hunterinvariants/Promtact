...
func manifestHMACKey() []byte {
	secret := strings.TrimSpace(os.Getenv("PROMTACT_MANIFEST_HMAC_SECRET"))
	if secret == "" {
		return nil
	}
	return []byte(secret)
...
func manifestSignatureRequired() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("PROMTACT_MANIFEST_REQUIRE_SIGNED"))) {
	case "1", "true", "yes", "on":
		return true
	default:
		return false
...
// SignThreatPackFile signs the manifest at path with PROMTACT_MANIFEST_HMAC_SECRET
// and writes a detached signature to path + ".sig". It returns the signature
// file path.
func SignThreatPackFile(path string) (string, error) {
	key := manifestHMACKey()
	if len(key) == 0 {
		return "", errors.New("PROMTACT_MANIFEST_HMAC_SECRET is required to sign a manifest")
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return "", err
...
// present. With PROMTACT_MANIFEST_REQUIRE_SIGNED set, an unsigned or unverifiable
...
func verifyThreatPackSignature(path string, data []byte) error {
	...
	if required {
		return errors.New("signed manifest required but PROMTACT_MANIFEST_HMAC_SECRET is not set")
	}
	log.Printf("warning: loading threat pack %q without signature verification; set PROMTACT_MANIFEST_HMAC_SECRET to enforce", path)
	return nil
}

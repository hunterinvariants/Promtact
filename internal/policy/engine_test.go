...
func TestGateToolCallVerifiesToolProvenance(t *testing.T) {
	engine := New(Config{
		...
		ToolProvenance: []ToolProvenanceEntry{{Tool: "signed_tool", Publisher: "promtact", Fingerprint: "sha256:good"}},
	})

	verified := engine.GateToolCall(domain.ToolCallRequest{
		ID: "p1", AssetID: "h", Actor: "a", ToolName: "signed_tool",
		Command: "list", ToolFingerprint: "sha256:good", ToolPublisher: "promtact",
	})
	...
	mismatch := engine.GateToolCall(domain.ToolCallRequest{
		...
		Command: "list", ToolFingerprint: "sha256:WRONG", ToolPublisher: "promtact",
	})

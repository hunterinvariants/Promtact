...
func TestPostgresAuditChainDetectsRecordTampering(t *testing.T) {
	...
	t.Setenv("PROMTACT_AUDIT_HMAC_SECRET", "integration-audit-secret")
	...
	if _, err := s.db.Exec(`DELETE FROM promtact_audit_events`); err != nil {
		t.Fatalf("clean audit events: %v", err)
	}
	if _, err := s.db.Exec(`UPDATE promtact_audit_chain_state SET head_hash = '', chain_index = 0, anchor_hmac = '' WHERE id = 1`); err != nil {
	...
	defer func() {
		for _, id := range ids {
			_, _ = s.db.Exec(`DELETE FROM promtact_audit_events WHERE id = $1`, id)
		}
	}()
	for _, id := range ids {
	...
	if _, err := s.db.Exec(`UPDATE promtact_audit_events SET data = jsonb_set(data, '{outcome}', '"tampered"') WHERE id = $1`, ids[0]); err != nil {
...
func TestPostgresAssetsAreTenantScoped(t *testing.T) {
	...
	defer func() {
		_, _ = s.db.Exec(`DELETE FROM promtact_assets WHERE id = $1`, assetID)
		_, _ = s.db.Exec(`DELETE FROM promtact_events WHERE id IN ($1, $2)`, evtA, evtB)
		_ = s.Close()
	}()
...
func cleanupPostgresIntegrationRows(t *testing.T, s *Store, ids ...string) {
	...
	for _, table := range []string{"promtact_events", "promtact_alerts", "promtact_actions", "promtact_audit_events", "promtact_assets"} {

...
func (s *Store) postgresLoadAssets(ctx context.Context) error {
	rows, err := s.db.QueryContext(ctx, `SELECT data FROM promtact_assets`)
	if err != nil {
		return err
	}
	defer rows.Close()
...
func (s *Store) postgresSyncAuditChainState(ctx context.Context) error {
	...
	if _, err := s.db.ExecContext(ctx, `
INSERT INTO promtact_audit_chain_state (id, head_hash, chain_index, valid, updated_at)
...
func (s *Store) postgresAuditChainSnapshot() AuditChainSnapshot {
	...
	if err := db.QueryRowContext(ctx, `SELECT head_hash, chain_index, valid, anchor_hmac FROM promtact_audit_chain_state WHERE id = 1`).Scan(&headHash, &chainIndex, &valid, &anchor); err != nil {
		s.mu.RLock()
		defer s.mu.RUnlock()
	...
	if err := db.QueryRowContext(ctx, `SELECT count(*) FROM promtact_audit_events`).Scan(&snap.Total); err != nil {
		s.mu.RLock()
		defer s.mu.RUnlock()
		return s.auditChainSnapshotLocked()
	}
	...
	if err := db.QueryRowContext(ctx, `SELECT id, occurred_at, data FROM promtact_audit_events WHERE data->>'hash' = $1 LIMIT 1`, snap.Head).Scan(&id, &ts, &data); err != nil {
		snap.Valid = false
		return snap
	}
	snap.LastAuditID = id
...
func (s *Store) postgresPersistAssetsLocked(ctx context.Context) error {
	for _, asset := range s.assets {
		...
		if _, err := s.db.ExecContext(ctx, `
INSERT INTO promtact_assets (id, last_seen, risk_score, data)
VALUES ($1, $2, $3, $4)
ON CONFLICT (id) DO UPDATE SET
  last_seen = EXCLUDED.last_seen,

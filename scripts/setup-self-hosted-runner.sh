...
repo_url="${REPO_URL:-https://github.com/hunterinvariants/promtact}"
repo_slug="${REPO_SLUG:-hunterinvariants/promtact}"
runner_name="${RUNNER_NAME:-sentinel-promtact}"
runner_labels="${RUNNER_LABELS:-promtact-staging}"
...
install -d -m 0755 /opt/promtact/bin
install -m 0755 "$script_dir/deploy-release.sh" /opt/promtact/bin/deploy-release.sh
install -m 0755 "$script_dir/promtact-deploy-wrapper.sh" /usr/local/sbin/promtact-deploy
chown root:root /opt/promtact/bin/deploy-release.sh /usr/local/sbin/promtact-deploy

cat >/etc/sudoers.d/promtact-runner <<EOF
$runner_user ALL=(root) NOPASSWD: /usr/local/sbin/promtact-deploy
EOF
chmod 0440 /etc/sudoers.d/promtact-runner
